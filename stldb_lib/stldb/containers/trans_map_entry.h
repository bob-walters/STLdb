...
/**
 * trans_set_hook is the additional data which is required on an entry
 * in a transactional container, and complimenets the use of set_base_hook
 */
class trans_set_hook {
public:
	// Default Constructor
	trans_set_hook()
		: _op(No_op), _txn_id(0), _checkpoint_location(0,0)
	{}
	
	trans_set_hook& operator=(const trans_set_hook& value) {
		_op = value._op;
		_txn_id = value._txn_id;
		// safety: protect against loss of checkpoint info via assignment
 ...
 */
template<typename T>
class TransEntry: public T, public trans_set_hook
{
public:
	...
	// update to new value without changing trans_set_hook details.
	TransEntry<T>&
	...
	void serialize(Archive &ar, const unsigned int version)
	{
	    ar & boost::serialization::base_object<T>(*this) 
			& boost::serialization::base_object<trans_set_hook>(*this);
	}
