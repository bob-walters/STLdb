...
/**
 * Note: This is a modified version of libstdc++-v3/include/bits/basic_string.h
 * from the GNU compiler collection (gcc) v 4.4.0.  It has undergone changes
 * which allows it to use allocator::pointer so that strings can safely reside
 * in shared memory mapped at different addresses by different processes.
 * It works with gcc compilers going back to 3.4.  The string class retains its copy-on-write,
 * reference counted approach (for performance), and use of atomic ints for
 * the reference count.
 *
 * Although packaged with STLdb,
 * it is a derived work of gcc, this file remains the property of GNU and is
 * subject to the terms of its original license, which is as follows:
 */
...
#ifndef _STLDB_BASIC_STRING_H
#define _STLDB_BASIC_STRING_H 1
  ...
  */
  ...
  template<typename _CharT, typename _Traits, typename _Alloc>
    ...
    private:
      ...
      _CharT*
      _M_data() const
      { return  __gnu_cxx::__static_pointer_cast<_CharT*>(_M_dataplus._M_p); }
	...
	return __testoff ? __off : this->size() - __pos;
      ...
      {
	return (std::less<const _CharT*>()(__s, _M_data())
	...
	return _M_data()[__pos];
      ...
      {
	if (__n >= this->size())
	  std::__throw_out_of_range(__N("basic_string::at"));
	return _M_data()[__n];
      }
	...
	return _M_replace_aux(__i1 - _M_ibegin(), __i2 - __i1, __n, __c);
      ...
      */
	  ...
	  typedef typename stldb::__is_integer<_InputIterator>::__type _Integral;
	...
	{
	  typedef typename stldb::__is_integer<_InIterator>::__type _Integral;
  ...
  template<typename _Traits, typename _Alloc>
    ...
    operator>>(std::basic_istream<char>& __is, basic_string<char,_Traits,_Alloc>& __str);
  ...
  template<typename _Traits, typename _Alloc>
    std::basic_istream<char>&
    getline(std::basic_istream<char>& __in, basic_string<char, _Traits, _Alloc>& __str,
	    char __delim);

#ifdef _GLIBCXX_USE_WCHAR_T
  template<typename _Traits, typename _Alloc>
    std::basic_istream<wchar_t>&
    getline(std::basic_istream<wchar_t>& __in, basic_string<wchar_t, _Traits, _Alloc>& __str,
	    wchar_t __delim);
#endif
...
#ifndef _GIBCXX_EXPORT_TEMPLATE
# include <algorithm> // for find_if
# include <stldb/containers/gnu/basic_string.tcc>
#endif
