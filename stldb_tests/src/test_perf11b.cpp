...
typedef stldb::concurrent::trans_map<shm_string, shm_string, 
	std::less<shm_string>,
	trans_map_allocator>  MapType;

static char buffer[1024];

struct entry_maker_11bt {
...
int test_perf11bload()
{
	...
	for (int i=0; i<database_count; i++) {
		test_ops.add( new insert_trans_operation<PartitionedTestDatabase<managed_mapped_file,MapType>,MapType,entry_maker_11bt>(*(dbs[i]), inserts_per_txn, maker), 100 );
	}
...
int test_perf11b()
{
	...
	for (int i=0; i<database_count; i++) {
		test_ops2.add( new find_trans_operation<PartitionedTestDatabase<managed_mapped_file,MapType>,MapType,entry_maker_11bt>(
				*(dbs[i]), max_key, finds_per_txn, maker), 60 );
		test_ops2.add( new update_trans_operation<PartitionedTestDatabase<managed_mapped_file,MapType>,MapType,entry_maker_11bt>(
...
{
	timer t11b("test_perf11b_readonly");
