    ...
    void forward() 
    {
        ...
        do {
          std::map<boost::interprocess::offset_t,std::size_t>::const_iterator i(_checkpoint.free_by_offset.lower_bound(_offset));
            ...
            // Skip any segment which appears invalid.  WARN about it, but don't
            // fail the load...
            if ((i != _checkpoint.free_by_offset.end() && 
                _offset + (ssize_t)size > i->first) 
                || (_offset + (ssize_t)size > _length))
            {
                if (i != _checkpoint.free_by_offset.end()) {
                    // Invalid entry detected - size is too large.
                    STLDB_TRACE(severe_e,"Checkpoint region with invalid size at offset=" << _offset << ".  Header size=" << size << " extends into/beyond the next free block. Skipping checkpoint file region " << _offset << " through " << i->first + i->second);
                    _offset = i->first;
                    continue;
                }
                else {
                    STLDB_TRACE(severe_e,"Checkpoint region with invalid size at offset=" << _offset << ".  Header size=" << size << " extends beyond the end of the file. Skipping remaining checkpoint file");
                    _offset = _length;
                    continue;
                }
            }
            else {
                // now check the checksum of the checkpoint entry
                uint32_t checksum;
                ...
                found_entry = true; // found a valid looking entry.
            }
        }
        while (!found_entry);
