...
public:
	...
	virtual void operator()() {
		...
		{ // lock scope
			int partition = last_key / 10;
			map_type& map = *db.getMap(partition);
...
public:
	find_trans_operation(database_type &db, int highest_key, int finds_per_txn, entry_maker_t &maker)
		: db(db), entry_maker(maker), highest_key(highest_key), txnSize(finds_per_txn), hits(0), misses(0)
		{ }
...
public:
	update_trans_operation(database_type &db, int highest_key, int txn_size, entry_maker_t &maker)
		: db(db), entry_maker(maker), highest_key(highest_key), txnSize(txn_size), hits(0), misses(0)
		{ }
