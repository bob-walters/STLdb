...
int test_perf10bload()
{
	...
	for (int i=0; i<database_count; i++) {
		test_ops.add( new insert_trans_operation<TestDatabase<managed_mapped_file,MapType>,MapType,entry_maker_10bt>(*(dbs[i]), inserts_per_txn, maker), 100 );
	}
...
int test_perf10b()
{
	...
	for (int i=0; i<database_count; i++) {
		test_ops2.add( new find_trans_operation<TestDatabase<managed_mapped_file,MapType>,MapType,entry_maker_10bt>(
			*(dbs[i]), max_key, finds_per_txn, maker), 60 );
		test_ops2.add( new update_trans_operation<TestDatabase<managed_mapped_file,MapType>,MapType,entry_maker_10bt>(
