	...
	while (i != end())
	{
		...
		for (int j=0; j<count; j++) {
			...
			if (i != this->baseclass::end() 
                // A row could also be deleted, and reinserted while the map was unlocked,
                // in which case the checkpoint location would have reverted to [0,0]
                && i->second.checkpointLocation().first == prev_locs[j].first ) 
            {
                ...
                BOOST_ASSERT( values[j].second.checkpointLocation().second > 0 );
