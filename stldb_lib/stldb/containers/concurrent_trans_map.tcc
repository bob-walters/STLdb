...
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::trans_map(const Comparator& comp, const Allocator& alloc, const char *name)
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::~trans_map()
{ }
...
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::while_row_locked(
		const bound_method_t &bound_method, iterator &i, Transaction &trans, wait_policy_t &wait_policy )
{
	...
	// Use the iterator which has been passed to us to acquire a lock on the
	// entry.  Calling operator*() does this, and the iterator now holds the
	// lock.  Also this is idempotent if i already has the lock.
	pointer entry = &*i;
	//boost::interprocess::scoped_lock<mutex_type> holder(_row_level_locks.mutex(&*i));
	t1.end();
	while (true) {
		try {
			...
			K temp = entry->first; // need to make a copy before retrying the lock, just in case
			uint64_t current_ver_num = this->_ver_num;
			wait_policy.unlock_container();
			try {
				wait_policy.wait(i.lock_held(), _row_level_lock_released);
			}
			catch(...) {
			...
			i.lock(); // will need _row_level_lock for next iteration.
		}
	}
}
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
typename trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::iterator
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::find(const K& key)
{
	...
	// acquire the row-level mutex to be sure about the value of 
	// i->second.getOperation() and not experience problems with reading 
	// a value which is in the progress of changing
	boost::interprocess::scoped_lock<mutex_type> row_holder(_row_level_locks.mutex(&*i));
	if ( i->second.getOperation() == Insert_op )
		return iterator( baseclass::end(), this );
	return iterator( i, this, row_holder );
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
typename trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::iterator
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::find(const K& key, Transaction &trans)
{
	...
	// acquire the row-level mutex to be sure about the value of i.endOfTransactionInProgress()
	// and not experience problems with reading a value which is in the progress of changing
	boost::interprocess::scoped_lock<mutex_type> row_holder(_row_level_locks.mutex(&*i));
	if ((i->second.getOperation() == Delete_op && i->second.getLockId() == trans.getLockId() ) ||
		(i->second.getOperation() == Insert_op && i->second.getLockId() != trans.getLockId() ))
		return iterator( baseclass::end(), this, trans );
	else
		return iterator( i, this, trans, row_holder );
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
typename trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::iterator
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::lower_bound(const K& key, Transaction &trans)
{
	stldb::timer t1("trans_map::lower_bound(key,trans)");
	typename baseclass::iterator i = baseclass::lower_bound(key);
	// can leave out check for Deleted_op, Insert_op, etc because the 
	// iterator's constructor will advance the position appropriately.
	return iterator( i, this, trans );
};
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
typename trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::iterator
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::upper_bound(const K& key, Transaction &trans)
{
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
void trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::lock_i( iterator &i, Transaction &trans )
{
	...
	if ( i->second.getLockId() != trans.getLockId() )
	{
		...
		addPendingUnlock(trans, i.base());
	}
}

template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
void trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::lock( iterator &i, Transaction &trans )
{
	stldb::timer t1("trans_map::lock(iterator,trans)");
	i.lock(); // lock the entry, prior to calling lock_i.
	lock_i( i, trans );
}
...
void trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::lock( iterator &i, Transaction &trans, wait_policy_t &wait_policy )
{
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
V& trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::update_i( iterator& i, const V &newValue,
		Transaction &trans )
{
	...
	switch ( i->second.getOperation() )
	{
		...
		case Update_op:
			...
			// stored in the map at the time of commit.
			{
				...
				addPendingUpdate(trans, i.base(), newValue);
	...
	return (pendingUpdate(trans, i.base()).second);
}


template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
V& trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::update( iterator& i, const V &newValue,
		Transaction &trans )
{
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
typename trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::iterator
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::insert( iterator i, const value_type& v, Transaction& trans )
{
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
	template <class wait_policy_t>
typename trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::iterator
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::insert( iterator i, const value_type& v,
		Transaction &trans, wait_policy_t &wait_policy )
{
	...
	while (true) {
		try {
			...
			uint64_t current_ver_num = this->_ver_num;
			{
				...
				typename baseclass::iterator uncommitted_row( baseclass::find(v.first) );
				boost::interprocess::scoped_lock<mutex_type> row_lock_holder(
						_row_level_locks.mutex(&*uncommitted_row));

				wait_policy.unlock_container();
				try {
					wait_policy.wait(row_lock_holder, _row_level_lock_released);
				}
				catch(...) {
					// to re-acquite the container-level lock, we have to first
					// unlock mutex, then lock container, to prevent mutex deadlocks
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
std::pair<typename trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::iterator, bool>
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::insert( const value_type& v, Transaction& trans )
{
	...
	if (result.second) {
		trans.insert_work_in_progress( this, new detail::map_insert_operation<trans_map>(*this, result.first) );
		addPendingInsert(trans, result.first);
	}
	else {
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
	template <class wait_policy_t>
std::pair<typename trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::iterator, bool>
trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::insert( const value_type& v,
		Transaction &trans, wait_policy_t &wait_policy )
{
	...
	while (true) {
		try {
			...
			try {
				...
				// to re-acquire the container-level lock, we have to first
				// unlock mutex, then lock container, to prevent mutex deadlocks
				if (row_lock_holder)
					row_lock_holder.unlock();
				wait_policy.relock_container();
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
void trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::erase_i( iterator& i, Transaction& trans )
{
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
void trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::erase( iterator i, Transaction& trans )
{
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
void trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::clear(exclusive_transaction& trans)
{
	/**
...
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
void trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::swap(trans_map &other, exclusive_transaction& trans)
{
	/**
	 * Important assumption underlying this methods design:
	 * 		boost::interprocess::map map1 = ...;
...
void trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::save_checkpoint(
		Database_t &db,
...
// load a checkpoint.
template <class K, class V, class Comparator, class Allocator, class mutex_family, int picket_lock_size>
void trans_map<K,V,Comparator,Allocator,mutex_family, picket_lock_size>::load_checkpoint(checkpoint_ifstream &checkpoint)
{
